...
/* ==================== إعدادات الجملة ====================
   ...
   ملاحظة مهمة: هذا الموقع يقرأ من نفس ورقة Data التي يقرأها موقع المفرد،
...
/* هل الموديل مؤهل للجملة؟ سعر صالح + عدد قطع كافٍ */
function isWholesaleItem(item){
  return hasWsPrice(item) && parseSizes(item.sizes).length >= MIN_PIECES;
}

/* تكلفة التوصيل */
function calcShip(){ return SHIP_COST; }
function isFreeShip(){ return SHIP_COST === 0; }

/* مجاميع السلة */
function cartPieces(){ return cart.reduce((s,i)=>s+Number(i.qty||0), 0); }
...
/* ===== بناء نص طلب الجملة — مجمّع حسب الموديل ===== */
function buildWholesaleMsg(name, phone, addr, notes){
  let msg='طلب جملة — المنتجات التالية:\n';
  cart.forEach(i=>{
    msg+='- ID: '+sanitize(String(i.id))+
         '، القطع: '+i.qty+
         '، القياسات: '+i.sizes.join('-')+
         '، سعر القطعة: '+Number(i.unit).toLocaleString()+' دينار'+
         '، الإجمالي: '+Number(i.total).toLocaleString()+' دينار\n';
  });
  const sub=cartTotal(), ship=calcShip(), total=sub+ship;
  msg+='\nعدد الموديلات: '+cart.length+'\n';
  msg+='عدد القطع: '+cartPieces()+'\n';
  msg+='مجموع البضاعة: '+sub.toLocaleString()+' دينار\n';
...
function startCountdown(){
  /* نسخة الجملة: لا عدّاد خصم — الدالة معطّلة بأمان */
  return;
}
...
if (_ckCopyBtn) _ckCopyBtn.addEventListener('click',()=>{
  ...
  const msg = buildWholesaleMsg(name, phone, addr, '');
  const total = cartTotal() + calcShip();
...
function updateFilterCounts(){
  ...
  /* نعدّ فقط الموديلات المؤهلة للجملة */
  allData.filter(isWholesaleItem).forEach(item=>{
...
function addJsonLD(products){
  ...
  const safe=products.filter(isWholesaleItem).slice(0,20);
  const schema={'@context':'https://schema.org','@type':'ItemList',name:'متجر جود العباس — جملة ملابس أطفال',itemListElement:safe.map((item,i)=>({'@type':'ListItem',position:i+1,item:{'@type':'Product',name:'منتج # '+item.Id,image:item.picture||'',offers:{'@type':'Offer',price:wsPrice(item),priceCurrency:'IQD',availability:'https://schema.org/InStock'}}}))};
...
function createCard(item){
  ...
  const bq=document.createElement('span'); bq.className='badge bqty';
  bq.textContent=qty+' '+t('pieces');
  badges.appendChild(bq);

  /* الصورة */
  const iw=document.createElement('div'); iw.className='cimg-w'; iw.appendChild(badges);
  const img=document.createElement('img'); img.className='cimg'; img.src=imgSrc; img.alt=String(item.Id);
  img.loading='lazy'; img.decoding='async'; img.fetchPriority='low';
  img.onerror=function(){this.src=PLACEHOLDER;};
  img.addEventListener('click',function(){openLB(this.src, item.Id, unit);});
  iw.appendChild(img);
  ...
  idEl.addEventListener('click',function(){
    const txt=String(item.Id);
    if(navigator.clipboard){ navigator.clipboard.writeText(txt).then(()=>notify(t('copyId')+': '+txt,'i')).catch(()=>{}); }
    else{ const ta=document.createElement('textarea'); ta.value=txt; document.body.appendChild(ta); ta.select();
          try{document.execCommand('copy');notify(t('copyId')+': '+txt,'i');}catch(_){} document.body.removeChild(ta); }
  });

  /* الأسعار: القطعة + إجمالي الموديل */
  const pr=document.createElement('div'); pr.className='cprices ws-prices';
  const up=document.createElement('span'); up.className='ws-unit';
  up.textContent=t('perPiece')+': '+unit.toLocaleString()+' '+t('dinar');
  const tp=document.createElement('span'); tp.className='ws-total';
  tp.textContent=t('modelTotal')+': '+total.toLocaleString()+' '+t('dinar');
  pr.appendChild(up); pr.appendChild(tp);

  /* القياسات المشمولة (عرض فقط) */
  const szLbl=document.createElement('div'); szLbl.className='ws-sz-lbl';
  szLbl.textContent=t('sizesIncluded')+':';
  const szList=document.createElement('div'); szList.className='ws-sz-list';
  sizes.forEach(sz=>{ const sp=document.createElement('span'); sp.className='ws-sz'; sp.textContent=getSzLbl(sz); szList.appendChild(sp); });

  /* زر إضافة الموديل كاملاً */
  const addBtn=document.createElement('button');
  addBtn.className='ws-add'+(inCart?' in-cart':'');
  ...
  body.appendChild(szLbl); body.appendChild(szList); body.appendChild(addBtn);
...
function displayProducts(arr){
  ...
  /* الجملة: سعر جملة صالح + عدد قطع كافٍ */
...
function shareCart(){
  ...
  let msg='🛍️ '+t('hTitle')+'\n\n';
  cart.forEach(ci=>{
    msg+='• #'+sanitize(String(ci.id))+' — '+ci.qty+' '+t('pieces')+' — '+Number(ci.total).toLocaleString()+' '+t('dinar')+'\n';
  });
  msg+='\n'+t('tot')+': '+(cartTotal()+calcShip()).toLocaleString()+' '+t('dinar');
  if(navigator.share) navigator.share({title:t('hTitle'),text:msg}).catch(()=>{});
...
/* الجملة: الموديل متوفر إذا بقي مؤهلاً (سعر + عدد قطع) */
function isItemAvailable(item){
  const prod = allData.find(p=>String(p.Id)===String(item.id));
  return !!prod && isWholesaleItem(prod);
}
...
function addModelToCart(item){
  ...
  if(cart.some(c=>String(c.id)===id)){ notify(t('already'),'w'); return; }
  if(!isWholesaleItem(item)){ notify(t('notEligible'),'w'); return; }
...
function renderCart(){
  ...
  cart.forEach(item=>{
    ...
    const img=document.createElement('img'); img.className='ci-img'; img.loading='lazy'; img.decoding='async';
    img.src=safeImgSrc(prod&&prod.picture?prod.picture:''); img.alt='';
    ...
    const ciQty=document.createElement('div'); ciQty.className='ci-qty';
    ciQty.textContent=item.qty+' '+t('pieces')+' · '+item.sizes.join(' · ');
    const ciUnit=document.createElement('div'); ciUnit.className='ci-unit';
    ciUnit.textContent=t('perPiece')+': '+Number(item.unit).toLocaleString()+' '+t('dinar');
    const ciTot=document.createElement('div'); ciTot.className='ci-new';
    ciTot.textContent=Number(item.total).toLocaleString()+' '+t('dinar');
    info.appendChild(ciId); info.appendChild(ciQty); info.appendChild(ciUnit); info.appendChild(ciTot);
  ...
  const pieces=cartPieces(), sub=cartTotal(), ship=calcShip(), total=sub+ship;
  const r1=document.createElement('div'); r1.textContent=t('models')+': '+cart.length+' · '+t('pieces')+': '+pieces;
  const r2=document.createElement('div'); r2.textContent=t('sub')+': '+sub.toLocaleString()+' '+t('dinar');
  ...
  if(MIN_MODELS>0 && cart.length<MIN_MODELS){
    const hint=document.createElement('div'); hint.className='ctot-hint';
    hint.textContent='📦 '+t('minModelsHint').replace('{n}', MIN_MODELS-cart.length);
    rows.push(hint);
  }
...
async function submitOrder(){
  ...
  if (unavailable.length > 0) {
    const ids = unavailable.map(i=>'#'+i.id).join('، ');
  ...
  const msg   = buildWholesaleMsg(name, phone, addr, notes);
  const total = cartTotal() + calcShip();
  ...
  const savePromise = saveOrderToGAS({
    ...
    /* الجملة: نُرسل كل موديل مع قياساته الفريدة —
       processOrder يحذف كل نسخ القياس، فيُفرَّغ الموديل بالكامل */
    items:   JSON.stringify(cart.reduce(function(acc,i){
               const uniq = Array.from(new Set(i.sizes.map(String)));
               uniq.forEach(function(s){ acc.push({id:String(i.id), size:s}); });
               return acc;
             }, []))
  });
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  const _px=document.getElementById('promoX'); if(_px) _px.addEventListener('click',()=>{const b=document.getElementById('promoBanner'); if(b) b.style.display='none';});
  fetchData();
